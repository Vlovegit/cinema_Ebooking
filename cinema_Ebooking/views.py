...
def activateEmail(request, user, to_email):
    ...
    if email.send():
        messages.info(request, f'Dear {fname}, please go to you email {to_email} inbox and click on \
            received activation link to confirm and complete the registration. Note: Check your spam folder.', extra_tags='verify')
    else:
...
def regisconfirmation(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        messages.info(request, 'Thank you for your email confirmation. Now you can login your account.', extra_tags='activated')
